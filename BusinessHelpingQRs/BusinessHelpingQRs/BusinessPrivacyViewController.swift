//
//  BusinessPrivacyViewController.swift
//  BusinessHelpingQRs
//
...
class BusinessPrivacyViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
    ...
    var privacyData: [Any]?
    @objc var url: String?
    let businessPrivacyPolicyUrl = "https://www.termsfeed.com/live/10f2dfc9-27c2-4aac-8916-56f0605741da"
    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.businessBlazeGetUserDefaultKey())
        initSubViews()
        initNavView()
        initWebView()
        businessStartLoadWebView()
    }
    ...
    */
    ...
    private func businessStartLoadWebView() {
        let urlStr = url ?? businessPrivacyPolicyUrl
    ...
    private func businessReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "BusinessPrivacyViewController") as? BusinessPrivacyViewController {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                businessSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                businessReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        businessSendEvent(tName, values: [tName: data])
                    }
                } else {
                    businessSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                businessAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    businessAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = businessJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = businessJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    businessSendEvents(withParams: evParams)
                }
            }
        }
    }
